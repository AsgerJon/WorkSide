...
class AttributeClass(WorkToyClass):
  ...
  _initLine = ['<tab=2>self.<pvtName> = <defVal>', ]
  ...
  def getCls(self, ) -> str:
    """Getter-function for class name"""
    return self._clsName

  def setCls(self, name: str) -> None:
    """Setter-function for class name"""
    self._clsName = name

  def setDoc(self, documentation: str) -> None:
    """Setter-function for documentation"""
    if isinstance(documentation, list):
      return self.setDoc('\n'.join(documentation))
    if isinstance(documentation, str):
      self._docstring = documentation
  ...
  def getParentName(self) -> str:
    """Getter-function for the parent name"""
    return self.getParent().__qualname__

  def getDocstring(self) -> str:
    """Getter-function for documentation"""
    if isinstance(self.getDoc(), str):
      return '\"\"\"%s\"\"\"' % self.getDoc()
    if isinstance(self.getDoc(), list):
      return '\"\"\"%s\"\"\"' % '\n'.join(self.getDoc())
